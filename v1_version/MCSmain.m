...
r = rand(nb+ng,1);
statetime(:,1) = ceil(-log(r)./Fail);  % 生成机组线路状态持续时间
windB = 1;%初始风速
wind = [Bcenters(windB);Bcenters(windB);Bcenters(windB);Bcenters(windB)];
Lreal = busLoad*1.2;
SystemEDNS = 0;SystemDNS = [];LOLP = 0;
while (VarianceCoefficient> 0.05)
    ...
    for i = 1:Dk
        ...
        rB = rand(12,1);
        windtime(:,1) = ceil(-log(rB)./pB(windB,:)');  % 切换风速状态
    ...
    if state(TranstionElementIndex,1) == 0
        statetime(TranstionElementIndex,1) = ceil(-log(r)./Fail(TranstionElementIndex,1));
    else
