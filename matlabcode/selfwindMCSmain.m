...
busLoad = (bus.MWLoad-bus.MVARLoad)/baseMVA;%节点负荷比例
...
Branch_X = [branch.X;0.1];%电抗%%%%%%%%%%%%%%%%%%%%%%%%%%%直流线路如何解决？
...
tic
while (VarianceCoefficient> 0.01 && s<100)
    ...
    xl = state(1:nb);
    xg = state(nb+1:nb+ng);
    DNS = 0;DNS2 = zeros(2);%用于判断故障发生的时刻
    %某机组状态下每个小时的最小削负荷
    for i = 1:Dk
