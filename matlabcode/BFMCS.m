...
windB = 1;windF = 1;%初始风速
wind = [Bcenters(windB);Fcenters(windF);Fcenters(windF);Fcenters(windF)];
SystemEDNS = 0;SystemDNS = [];LOLP = 0;LOLF = 0;
DNS2 = zeros(2,1);%用于判断故障发生的时刻
proba = zeros(ng,1);proba_temple = zeros(ng,1);%机组使用率
tic
while (VarianceCoefficient> 0.01 && s<500)
    ...
    for i = 1:Dk
        ...
        proba = proba + proba_temple;
        proba_temple = zeros(ng,1);
        rB = rand(12,1);
        windtimeB(:,1) = -log(rB)./pB(windB,:)';  % 切换B风速状态
        [~,windB] = min(windtimeB(:,1));
        rF = rand(12,1);
    ...
    DNS2(1,1) = DNS2(2,1);DNS2(2,1) = DNS;
